...
from src.override_class import T5CG_Mod
import numpy as np

class FiDT5(T5CG_Mod):
    ...
    def get_prob(self, input_ids=None, attention_mask=None, labels=None, label_lens=None, **kwargs):
        ...
        # fix the input_id if the eos token is missing
        for i, seqlen in enumerate(label_lens):
            end_pos = seqlen-1
            if labels[i][end_pos] != 1:
                labels[i][end_pos] = 1

        token_celoss = super().forward(
                input_ids=input_ids,
        ...
        # logprob -> filtering -> sum -> prob
        token_logprob = -token_celoss # (bsz * ans_maxlen, )
        token_logprob = token_logprob.view(input_ids.shape[0], -1) # bsz, ans_maxlen     
        for i in range(label_lens.shape[0]):
            token_logprob[i,int(label_lens[i]):] = 0
        seq_logprob = torch.sum(token_logprob, dim=-1) # bsz
        seq_logprob = seq_logprob / label_lens # length normalization
    ...
    # dimensions used in the decoder.
    # EncoderWrapper resizes the inputs as (B * N) x L.
    def forward(self, input_ids=None, attention_mask=None, labels=None, cand_arg=None, selection_arg=None, selection_tuning=False, **kwargs):
        
        if not selection_tuning:
            if input_ids != None:
                # inputs might have already be resized in the generate method
                if input_ids.dim() == 3:
                    self.encoder.n_passages = input_ids.size(1)
            ...
            assert selection_arg is not None and  cand_arg is not None, "forward arg error"

            single_batch = cand_arg
            unified_batch = selection_arg
            
            # STEP 1 - unpack unified batch
            _, context_ids, context_mask, _, _, _, _ = unified_batch
            bsz = context_ids.shape[0]
            
            # STEP 2 - get probs for candidates
            cand_probs = torch.zeros(bsz).cuda()
            cand_ems = torch.zeros(bsz).cuda()
            
            # unpack single batch
            _, _, _, cand_seq, _, seq_lens, ems = single_batch

            cand_probs[:] = self.get_prob(input_ids=context_ids,
            ...
            # STEP 3 - get MSELoss
            cand_probs = cand_probs.view(-1) # bsz * 3
            cand_ems = cand_ems.view(-1) # bsz * 3
            loss_fct = MSELoss()
            loss = loss_fct(cand_probs, cand_ems)
            return (loss,)

    # We need to resize the inputs here, as the generate method expect 2D tensors
    def generate(self, input_ids, attention_mask, max_length,output_greedy_score=True):
        self.encoder.n_passages = input_ids.size(1)
    ...
    # We need to resize the inputs here, as the generate method expect 2D tensors
    def cand_generate(self, input_ids, attention_mask, max_length, num_beams=1):
        self.encoder.n_passages = input_ids.size(1)
        return super().cand_generate(
            input_ids=input_ids.view(input_ids.size(0), -1),
            attention_mask=attention_mask.view(attention_mask.size(0), -1),
            max_length=max_length,
            num_beams=num_beams,
...
class EncoderWrapper(torch.nn.Module):
    ...
    def forward(self, input_ids=None, attention_mask=None, **kwargs,):
        ...
        #print('input_ids given to encoder:',input_ids.shape)
