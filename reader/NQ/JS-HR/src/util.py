...
def set_optim(opt, model, warmup_steps=1000, scheduler_steps=15000):
    if opt.optim == 'adam':
    ...
    if opt.scheduler == 'fixed':
        ...
        scheduler = WarmupLinearScheduler(optimizer, warmup_steps=warmup_steps, scheduler_steps=scheduler_steps, min_ratio=0., fixed_lr=opt.fixed_lr)
    return optimizer, scheduler


# average_main, sum_main, weighted_average modified for deepspeed
def average_main(x, opt, world_size):
...
def weighted_average(x, count, opt):
    ...
    t_loss = torch.tensor([x * count], device=local_device)
    t_total = torch.tensor([count], device=local_device)
