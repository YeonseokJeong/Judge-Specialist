...
def get_beam_result(all_result, bsz, beam_idx, num_beams):
    beam_result = torch.zeros_like(all_result)[:bsz]
    for i in range(bsz):
        beam_result[i] = all_result[i*num_beams + beam_idx]
    return beam_result
...
def evaluate(model, eval_batches_list, tokenizer, collator, opt, step):
    ...
    with torch.no_grad():
        for b_idx in sampler:
            ...
            cand_selfprobs = torch.zeros(bsz, cand_num*num_beams, dtype=float)
            cand_probs = torch.zeros(bsz, cand_num*num_beams, dtype=float)
            cand_ems = torch.zeros(bsz, cand_num*num_beams, dtype=float)
            for i, single_batch in enumerate(single_batches):
                # unpack single batch
                _, _, _, cand_seq, seq_scores, seq_lens, ems = single_batch;seq_lens=seq_lens.long()
                
                for j in range(num_beams):
                    cand_seq_beam = get_beam_result(cand_seq, bsz, j, num_beams)
                    seq_scores_beam = get_beam_result(seq_scores, bsz, j, num_beams)
                    seq_lens_beam = get_beam_result(seq_lens, bsz, j, num_beams)
                    ems_beam = get_beam_result(ems, bsz, j, num_beams)
                    beam_prob = model.get_prob(input_ids=context_ids.cuda(),
                                                   attention_mask=context_mask.cuda(),
                                                   labels=cand_seq_beam.cuda(),
                                                   label_lens=seq_lens_beam.cuda())
                    cand_probs[:,i*num_beams + j] = beam_prob
                    
                    
                    seq_logprobs = seq_scores_beam.cuda()
                    cand_selfprobs[:,i*num_beams + j] = torch.exp(seq_logprobs)
                    cand_ems[:,i*num_beams + j] = ems_beam
